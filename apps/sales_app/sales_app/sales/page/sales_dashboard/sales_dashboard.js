frappe.pages["sales-dashboard"].on_page_load = function (wrapper) {
	...
	const sections = [
		{
			...
			doctype: "Lead",
			color: "#2563eb",
			open_filters: { status: ["in", ["Open", "Replied", "Interested", "Opportunity"]] },
		},
		{
			key: "opportunities",
			label: "Opportunities",
			doctype: "Opportunity",
			color: "#7c3aed",
			open_filters: { status: ["in", ["Open", "Quotation"]] },
		},
		{
			key: "quotations",
			label: "Quotations",
			doctype: "Quotation",
			color: "#0891b2",
			open_filters: { status: ["in", ["Draft", "Open"]] },
		},
		{
			key: "orders",
			label: "Orders",
			doctype: "Sales Order",
			color: "#ea580c",
			open_filters: { status: ["in", ["To Deliver and Bill", "To Bill", "To Deliver"]] },
		},
		{
			key: "deliveries",
			label: "Deliveries",
			...
			color: "#0ea5e9",
			open_filters: { status: ["in", ["Draft", "To Bill"]] },
		},
		{
			key: "invoices",
			label: "Invoices",
			...
			color: "#dc2626",
			open_filters: { status: ["in", ["Unpaid", "Partially Paid", "Overdue"]] },
		},
		{
			key: "payments",
			label: "Payments",
	...
	const style = $(`
		<style>
			...
			.sales-dash-actions { display: flex; gap: 10px; flex-wrap: wrap; margin: 8px 0 12px; align-items: center; }
			.sales-dash-actions .btn { border-radius: 999px; }
			.sales-dash-actions .btn:hover { transform: translateY(-1px); }
			.sales-period-filters { display: flex; gap: 8px; margin: 2px 0 12px; }
			.sales-period-btn.active { background: var(--gray-900); color: #fff; border-color: var(--gray-900); }
			.sales-period-btn { border-radius: 999px; }
			...
			.sales-kpi:hover::after { transform: translateX(100%); }
			...
			.sales-kpi .k-total { font-size: 12px; color: var(--text-muted); margin-bottom: 6px; font-weight: 600; }
			...
			@media (max-width: 1399px) { .sales-kpi-grid { grid-template-columns: repeat(4, minmax(0, 1fr)); } }
	...
	const body = $(`
		<div class="sales-dash-wrap">
			<div class="sales-dash-actions">
				...
				<button class="btn btn-default btn-sm" id="sales-new-quotation">${__("New Quotation")}</button>
				<button class="btn btn-default btn-sm" id="sales-new-order">${__("New Order")}</button>
				<button class="btn btn-default btn-sm" id="sales-new-delivery">${__("New Delivery")}</button>
				<button class="btn btn-default btn-sm" id="sales-new-invoice">${__("New Invoice")}</button>
				<button class="btn btn-default btn-sm" id="sales-new-payment">${__("New Payment")}</button>
				<button class="btn btn-default btn-sm" id="sales-refresh">${__("Refresh")}</button>
			</div>
			<div class="sales-period-filters">
				<button class="btn btn-default btn-xs sales-period-btn active" data-period="month">${__("This Month")}</button>
				<button class="btn btn-default btn-xs sales-period-btn" data-period="quarter">${__("This Quarter")}</button>
				<button class="btn btn-default btn-xs sales-period-btn" data-period="year">${__("This Year")}</button>
			</div>
	...
	const formatMoney = (value) => format_currency(toFloat(value), frappe.defaults.get_default("currency"));
	...
	const renderWithRecharts = (counts) => {
		...
		const data = sections.map((s) => ({
			...
			total: toFloat(counts.totals?.[s.key]),
			doctype: s.doctype,
			color: s.color,
			open_filters: s.open_filters,
		}));
			...
			data.map((item) =>
				e(
					"div",
					{
						...
						onClick: () => frappe.set_route("List", item.doctype, "List", item.open_filters || {}),
					},
					...
					e("div", { className: "k-total" }, `${__("Total")}: ${formatMoney(item.total)}`),
					e(
						"div",
						{ className: "k-meta" },
		...
		root.render(e("div", null, kpiNode, e("div", { className: "sales-re-grid-2" }, barChartNode, pieChartNode)));
	...
	const renderWithFrappeCharts = (counts) => {
		...
		const kpiHtml = data
			.map((item) => {
				...
				return `
					<div class="sales-kpi" data-doctype="${frappe.utils.escape_html(item.doctype)}" data-open-filters='${JSON.stringify(item.open_filters || {})}' style="border-left: 4px solid ${item.color};">
						<div class="k-label">${item.name}</div>
						<div class="k-top"><div></div><i data-lucide="bar-chart-3"></i></div>
						<div class="k-value">${item.value}</div>
						<div class="k-total">${__("Total")}: ${formatMoney(item.total)}</div>
		...
		kpiEl.querySelectorAll(".sales-kpi").forEach((card) => {
			card.addEventListener("click", () => {
				let openFilters = {};
				try {
					openFilters = JSON.parse(card.dataset.openFilters || "{}");
				} catch (e) {
					openFilters = {};
				}
				frappe.set_route("List", card.dataset.doctype, "List", openFilters);
			});
		});
		activateLucideIcons();
	};
	...
	const loadDashboard = async () => {
		...
		try {
			const r = await frappe.call({
				method: "sales_app.api.get_sales_dashboard_counts",
				args: { period: selectedPeriod },
			});
	...
	const setPeriodFilter = (period) => {
		...
		body.find(".sales-period-btn").removeClass("active");
		body.find(`.sales-period-btn[data-period="${period}"]`).addClass("active");
		loadDashboard();
	};
	...
	body.find("#sales-new-quotation").on("click", () => frappe.new_doc("Quotation"));
